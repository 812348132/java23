                    ...
                    <table class="table table-hover" >
                        ...
                        <c:forEach items="${diskList}" var="disk">
                            <tr class="tr" rel="${disk.id}">
                                ...
                                <td>${disk.name}</td>
                                <td><fmt:formatDate value="${disk.updateTime}" type="date" dateStyle="long"/> </td>
                                <td width="100">
                                    <c:if test="${disk.type == 'file'}">
                                        ${disk.fileSize}
                                    </c:if>
                                ...
                                <td width="150">
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu">
                                            <c:if test="${disk.type == 'file'}">
                                                <li><a href="javascript:;" rel="${disk.id}" class="rename">重命名</a></li>
                                                <li><a href="">删除</a></li>
                                            </c:if>
                                            <c:if test="${disk.type == 'dir'}">
                                                <li><a href="/disk?_=${disk.id}">打开</a></li>
                                                <li><a href="javascript:;" rel="${disk.id}" class="rename">重命名</a></li>
                                                <li><a href="javascript:;" rel="${disk.id}" class="del">删除</a></li>
                                            </c:if>
                                        </ul>
                                    </div>
                                </td>
    ...
    <tr class="tr" rel="{{id}}">
        ...
        <td width="100">
            <div class="btn-group">
                ...
                <ul class="dropdown-menu">
                    <? if(type == 'file') { ?>
                    <li><a href="javascript:;" rel="{{id}}" class="rename">重命名</a></li>
                    <li><a href="">删除</a></li>
                    <? } else if(type == 'dir') { ?>
                    <li><a href="/disk?_={{id}}">打开</a></li>
                    <li><a href="javascript:;" rel="{{id}}" class="rename">重命名</a></li>
                    <li><a href="javascript:;" rel="{{id}}" class="del">删除</a></li>
                    <?}?>
                </ul>
            </div>
        </td>
...
<script>
    $(function () {
                ...
                $.post("/disk/new/folder",{"pId":pid,"name":text,"accountId":accountId}).done(function(resp){
                    if(resp.state == 'success') {
                        ...
                        for(var i = 0;i < resp.data.length;i++) {
                            ...
                            obj.updateTime = moment(obj.updateTime).format("YYYY年MM月DD日"); //将时间戳格式化
        ...
        $(document).delegate(".rename","click",function (event) {
            ...
            layer.prompt({title:"请输入新的文件名"},function (text,index) {
                layer.close(index);
                $.post("/disk/rename",{"id":id,"name":text}).success(function (data) {
        ...
        $(document).delegate(".del","click",function (event) {
            ...
            layer.confirm("确定要删除吗?",function (index) {
                layer.close(index);
                $.post("/disk/del",{"id":id}).success(function (data) {
                    if(data.state == "success"){
                        window.history.go(0);
                    } else {
                        layer.msg(data.message);
